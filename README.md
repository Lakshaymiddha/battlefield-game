...
- **Battlefield API** – Initialize battlefield, add ships, start the game.
...
- **Graceful Exception Handling** – Invalid actions (duplicate ship, invalid placement, firing outside bounds) do not crash the game but return meaningful results.
- **Full Testing** – Unit and integration tests ensure correctness and resilience.
...
### 1. Builder Pattern
- Used in `ShipBuilder` and `PositionBuilder` for flexible creation with validation.
- Example:
2. Factory Method
Used in ShipFactory to create ships uniformly and encapsulate construction logic.

3. Strategy Pattern
FireStrategy interface with multiple implementations like RandomFireStrategy, SmartFireStrategy.

Allows switching firing logic without changing game core.

4. Observer Pattern
GameObserver interface notified on hits, misses, or ship destruction.

Supports logging, UI updates, or multiplayer notifications.
...
## Run application
```
mvn exec:java -Dexec.mainClass="com.battleship.BattleshipApplication"
```